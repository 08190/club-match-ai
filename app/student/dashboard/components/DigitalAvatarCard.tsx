    ...
    <div className="sticky top-20 space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 hover:shadow-md transition-shadow">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-slate-500">高质量匹配度: 98%</p>
        </div>

        {/* Download Profile Button */}
        <button className="w-full mb-6 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-lg font-medium transition-all shadow-sm">
        ...
        <div className="bg-slate-50 rounded-lg p-4 text-center border border-slate-100">
          <p className="text-xs text-slate-600 mb-1">人格类型</p>
          <p className="text-2xl font-bold text-blue-600">
            ENFP
          </p>
          <p className="text-xs text-slate-600 mt-1">热情的倡导者</p>
        </div>
      </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 text-center">
          {profile.skills.map((skill) => (
            <div key={skill.name} className="px-2 py-2">
              <p className="text-xs text-slate-600 mb-1">{skill.name}</p>
